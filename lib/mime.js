...
const { E, codes: ErrorCodes } = require('internal/errors');
E(
...
const { ERR_INVALID_MIME_SYNTAX } = ErrorCodes;

function HardenedMatch(str) {
  return RegExpPrototypeSymbolMatch(this, str);
}

function HardenedMatchAll(str) {
  return RegExpPrototypeSymbolMatchAll(this, str);
}

function HardenedReplace(str, replacer) {
  return RegExpPrototypeSymbolReplace(this, str, replacer);
}

function HardenedSearch(str) {
  return RegExpPrototypeSymbolSearch(this, str);
}

function HardenedSplit(str) {
  return RegExpPrototypeSymbolSplit(this, str);
}

let hardenRegExp = (pattern) => {
  pattern[SymbolMatch] = HardenedMatch;
  pattern[SymbolMatchAll] = HardenedMatchAll;
  pattern[SymbolReplace] = HardenedReplace;
  pattern[SymbolSearch] = HardenedSearch;
  pattern[SymbolSplit] = HardenedSplit;
  return pattern;
};
const MapIteratorPrototypeNext = primordials.uncurryThis(
  new Map().entries().next
);

const NotHTTPTokenCodePoint = hardenRegExp(/[^!#$%&'*+\-.^_`|~A-Za-z0-9]/g);
const NotHTTPQuotedStringCodePoint = hardenRegExp(
  /[^\t\u0020-~\u0080-\u00FF]/g
);
...
function toASCIILower(str) {
  return StringPrototypeReplace(str, ASCII_LOWER, (c) =>
...
function parseTypeAndSubtype(str) {
  ...
  const invalidTypeIndex = StringPrototypeSearch(
    trimmedType,
  ...
  const invalidSubtypeIndex = StringPrototypeSearch(
    trimmedSubtype,
    NotHTTPTokenCodePoint
  );
...
const QUOTED_VALUE_PATTERN = hardenRegExp(
  /^(?:([\\]$)|[\\][\s\S]|[^"])*(?:(")|$)/u
);
...
function parseParametersString(str, position, paramsMap = new Map()) {
  ...
  while (position < endOfSource) {
    ...
    if (
      ...
      StringPrototypeSearch(parameterValue, NotHTTPQuotedStringCodePoint) ===
        -1 &&
